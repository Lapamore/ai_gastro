...
from src.core.models import ChatMessage, AIProviderResponse, UserPreferencesData # AIProviderResponse обновлен
...
class OpenAIAITunnelService(AbstractAIService):
    def __init__(self, api_key: str, base_url: str, model_name: str, system_prompt_file: Optional[str] = None):
        ...
        if system_prompt_file:
            try:
                ...
                logger.error(f"Ошибка при загрузке системного промпта из {system_prompt_file}: {e}. Используется fallback.")
    ...
    async def get_ai_response(
        ...
        logger.info(f"Запрос к AITunnel API. Модель: {self.model_name}. Сообщений: {len(openai_messages)}")
        ...
        try:
            ...
            # Парсим тег [YOUTUBE_SEARCH: "..."] (регистронезависимо)
            match = re.search(r"\[YOUTUBE_SEARCH:\s*\"(.*?)\"\s*\]", ai_text_reply, re.IGNORECASE)
            if match:
                video_search_query = match.group(1).strip()
                # Удаляем тег из основного ответа AI, чтобы он не отображался пользователю
                ai_text_reply = ai_text_reply.replace(match.group(0), "").strip()
                logger.info(f"AI запросил поиск видео на YouTube: '{video_search_query}'")
            else:
                logger.info("AI НЕ запросил поиск видео (тег не найден в ответе).")
            
            return AIProviderResponse(reply=ai_text_reply, trigger_video_search_query=video_search_query)

        except Exception as e:
            logger.error(f"Ошибка при взаимодействии с AITunnel (OpenAI SDK): {e}", exc_info=True)
            return AIProviderResponse(reply="Извините, сервис AI временно недоступен.", trigger_video_search_query=None)
    ...
    async def get_personalized_suggestions(
        ...
        effective_system_prompt = system_prompt or self.base_system_prompt
        preferences_text_parts: List[str] = [] # Явная типизация
        if preferences:
            if preferences.allergies: preferences_text_parts.append(f"- Аллергии: {', '.join(preferences.allergies)}.")
            if preferences.dietary_restrictions: preferences_text_parts.append(f"- Диета: {', '.join(preferences.dietary_restrictions)}.")
            if preferences.favorite_cuisines: preferences_text_parts.append(f"- Любимые кухни: {', '.join(preferences.favorite_cuisines)}.")
            # ... другие предпочтения ...
        
        preferences_text_for_prompt = "" # Инициализация
        if preferences_text_parts: 
             preferences_text_for_prompt = "Мои предпочтения:\n" + "\n".join(preferences_text_parts)
        
        history_summary = "Ранее мы обсуждали:\n" + "\n".join([f"{'Я' if m.sender=='user' else 'Ты'}: {m.text}" for m in conversation_history[-6:]]) if conversation_history else "Это наш первый диалог."
        ...
        openai_messages.append({"role": "user", "content": recommendation_prompt_content})

        try:
            chat_completion = await self.async_client.chat.completions.create(
                messages=openai_messages, model=self.model_name, max_tokens=500, temperature=0.8, n=1
            )
            raw_reply = chat_completion.choices[0].message.content.strip() if chat_completion.choices and chat_completion.choices[0].message else ""
            # Улучшенный парсинг предложений
            suggestions = []
            if raw_reply:
                # Сначала пытаемся найти строки, начинающиеся с цифры и точки, или маркера списка
                potential_suggestions = re.findall(r"^\s*(\d+\.\s*.*?|- \s*.*?|\* \s*.*?)$", raw_reply, re.MULTILINE)
                if potential_suggestions:
                    suggestions = [s.strip() for s in potential_suggestions]
                else: # Если не нашли четких маркеров списка, делим по новым строкам, если их несколько
                    lines = [line.strip() for line in raw_reply.split('\n') if line.strip()]
                    if len(lines) > 1 : # Если это действительно несколько предложений
                        suggestions = lines
                    elif raw_reply: # Иначе берем все как одно предложение
